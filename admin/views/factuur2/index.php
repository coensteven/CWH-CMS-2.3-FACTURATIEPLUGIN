...
<!-- index -->
<?php
$search_btn2 = '<div class="btn-group">';
$search_btn2 .= form_button ( "<i class='fa fa-search'></i> Zoeken2", array (
        'onclick' => 'Factuurklanten_Search_modal()',
        'style' => 'display:none',
...
$src_param = 'admin-page=factuur&sub_page=factuur2';
$search_btn2 .= form_button ( "<i class='fa fa-times'></i>", array (
        'onclick' => 'cancel_Factuurklanten_Search()',
        'style' => 'display:none',
        'class' => 'btn btn-default btn-index' 
) );
$search_btn2 .= '</div>';
add_toolbar_button ( $search_btn2 );
?>
<?php add_toolbar_button(form_button("<i class='fa fa-user'></i> Voeg klant toe", array('onclick' => 'Factuur_Klantenadd();','style' => 'display:none', 'class' => 'btn btn-primary btn-index'))); ?>
<!-- /index -->

<!-- add -->
<?php add_toolbar_button(form_button("<i class='fa fa-check-circle'></i> Opslaan", array('onclick' => 'Factuur2_New();','style' => 'display:none', 'class' => 'btn btn-primary btn-add'))); ?>
...
<!-- edit -->
<?php add_toolbar_button(form_button("<i class='fa fa-check-circle'></i> Opslaan", array('onclick' => 'Factuur2_Edit();','style' => 'display:none', 'class' => 'btn btn-primary btn-edit'))); ?>
<?php add_toolbar_button(form_button("<i class='fa fa-check-circle'></i> Toepassen", array('onclick' => 'Factuur2_Apply();','style' => 'display:none', 'class' => 'btn btn-info btn-edit'))); ?>
...
<div class="row" style="padding: 10px 0px;">
    <div class="col-md-6">
        <div class="btn-group">
            ...
            <ul class="dropdown-menu" role="menu">
                <li><a
                    href="<?php echo admin_url("admin-page=factuur&sub_page=factuur2", false); ?>"> <?php echo __('All', 'testimonials'); ?></a></li>
                                ...
                                <th style="background: #3c8dbc; color: white;width: 137px;">
                                    ...
                                    <div style="cursor: pointer; position: relative;" id="order_factuur_naam" onclick="Testimonials_Oder(this,'factuur_naam');">
                                        ...
                                        <i class="fa order-icon <?php echo $testimonial_name_icon; ?>" style="position:absolute; top:4px; right:0; <?php if ($testimonial_name_icon == ""){ ?> display:none;<?php } ?>"></i>
															  ...
															  <th style="background: #3c8dbc; color: white;width:88px">Bedrijf</th>
                                <th style="background: #3c8dbc; color: white;width: 235px;">
                                    <?php
                                    ...
                                    <div style="cursor: pointer; position: relative;" id="order_factuur_content" onclick="Testimonials_Oder(this,'factuur_content');">
                                        ...
                                        <i class="fa order-icon <?php echo $testimonial_content_icon; ?>" style="position:absolute; top:4px; right:0; <?php if ($testimonial_content_icon == ""){ ?> display:none;<?php } ?>"></i>
															  ...
															  <th style="background: #3c8dbc; color: white;width:44px">Klant ID</th>
                                <th style="background: #3c8dbc; color: white;width:44px">Klant ID (zelf)</th>
                        ...
                        foreach ( $this->records as $index => $record ) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $record["factuur_naam"]; ?></td>
															  <td><?php echo $record["factuur_bedrijfsnaam"]; ?></td>
                                <td><?php echo $record["factuur_content"]; ?></td>
                                <td><?php echo $record["id"]; ?></td>
															  <td><?php echo $record["factuur_klant"]; ?></td>
                                <td>
                                    <?php if($record['factuur_status'] == 1){ ?>
                                        <span id="status_<?php echo $record['id'];?>"><a href="javascript:change_statusklanten(<?php echo $record['id'];?>,'unpublish');" class="label label-success">Actief</a></span>
                                    <?php }else{?>
                                        <span id="status_<?php echo $record['id'];?>"><a href="javascript:change_statusklanten(<?php echo $record['id'];?>,'publish');" class="label label-danger">Niet actief</a></span>
                                    <?php }?>
                                </td>

                                <td class="text-center">
                                    <button type="button"
                                        onclick="Factuur2_Edit('<?php echo (int)$record['id'];?>','<?php echo $token; ?>');"
                                        class="btn btn-info btn-xs">
                                        <i class="fa fa-pencil"></i>
                                    </button>
                                    <button type="button"
                                        onclick="Factuur2_Delete('<?php echo (int)$record['id'];?>','<?php echo $token; ?>');"
...
<div class="modal fade" id="search_modal2" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form name="frm_Factuurklanten_Search"
                id="frm_Factuurklanten_Search" class="form-horizontal"
            ...
            <div class="modal-body">
                ...
                <?php echo Pf::event()->trigger("filter","factuurklanten-search-form"); ?>
            </div>
            </form>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="Factuurklanten_Search();">
...
function Factuurklanten_Search(){
	$("#main-content").mask("<?php echo __('Loading...','testimonials'); ?>");
	$('#search_modal2').modal('hide');
	$('#search_modal2').on('hidden.bs.modal', function (e) {
		$.get('<?php echo admin_url($src_param,false); ?>',$('#frm_Factuurklanten_Search').serialize(),function(html){
...
function Factuur2_Edit(id,token){
	$("#main-content").mask("<?php echo __('Loading...','testimonials'); ?>");
...
function Factuur2_Delete(id,token){
	$.sModal({
    	image:'<?php echo RELATIVE_PATH; ?>/app/plugins/default/theme/admin/images/confirm.png',
